...
class SessionSummaryClient
  ...
  def summarize(transcript)
    ...
    raise CodexExecRunner::ExecutionError, "session summary returned an empty reply" if summary_text.empty?

    summary_text
  rescue JSON::ParserError => e
    raise CodexExecRunner::ExecutionError, "session summary returned invalid JSON: #{e.message}"
  end
