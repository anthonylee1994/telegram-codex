class Telegram::WebhookHandler
  ...
  TOO_MANY_IMAGES_MESSAGE = "你一次過畀太多圖，我未必可以準確逐張睇。揀最多 10 張最關鍵嘅圖，或者直接講明想我集中比較邊幾張、邊一方面。"
  ...
  def handle(update)
    ...
    message = @media_group_aggregator.call(message)
    return if message.equal?(Telegram::MediaGroupAggregator::DEFERRED)

    handle_inbound_message(message, update: update)
  end

  def handle_inbound_message(message, update: nil)
