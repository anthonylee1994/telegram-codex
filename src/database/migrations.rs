...
/// Port of `CreateAppTables1700000000000`. The statements are idempotent so they
/// keep working against databases created by the TypeORM version.
...
pub async fn run(pool: &SqlitePool) -> Result<()> {
    for statement in STATEMENTS {
        sqlx::query(statement).execute(pool).await.with_context(|| format!("Migration statement failed: {statement}"))?;
    }
    Ok(())
}
