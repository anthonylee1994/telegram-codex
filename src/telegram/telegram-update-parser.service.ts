...
export class TelegramUpdateParser {
    public parseIncomingTelegramMessage(update: unknown): IncomingTelegramMessage | null {
        if (!this.isTelegramSupportedMessage(update)) {
            return null;
        }
    ...
    private isTelegramSupportedMessage(update: unknown): update is TelegramSupportedMessage {
