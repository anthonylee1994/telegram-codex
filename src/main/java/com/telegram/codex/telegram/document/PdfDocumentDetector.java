...
@Component
public class PdfDocumentDetector implements DocumentTypeDetector {
    ...
    public boolean supports(Map<String, Object> document) {
        ...
        if (DocumentConstants.PDF_MIME_TYPE.equals(mimeType)) {
            return true;
        }
        String fileName = MapUtils.stringValue(document.get("file_name")).toLowerCase();
        return fileName.endsWith(DocumentConstants.PDF_EXTENSION);
    }
