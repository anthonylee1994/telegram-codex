...
import type {GenerateReplyInput, GenerateReplyResult} from "../conversation/conversation.types.js";
import type {ReplyClient} from "../config/service.types.js";
...
export class CodexCliClient implements ReplyClient {
    public async generateReply(input: GenerateReplyInput): Promise<GenerateReplyResult> {
        const transcript = this.parseConversationState(input.conversationState);
        const userMessage = input.text || input.imageFilePath ? input.text || "請描述呢張圖，並按我需要幫我分析。" : "";
        const nextTranscript = this.trimTranscript([...transcript, {role: "user", content: userMessage}]);
        const prompt = this.buildPrompt(nextTranscript, Boolean(input.imageFilePath));
        const text = await this.runCodexExec(prompt, input.imageFilePath ?? null);
        const updatedTranscript = this.trimTranscript([...nextTranscript, {role: "assistant", content: text}]);
    ...
    private parseConversationState(conversationState: string | null): TranscriptMessage[] {
    ...
    private trimTranscript(transcript: TranscriptMessage[]): TranscriptMessage[] {
        return transcript.slice(-MAX_TRANSCRIPT_MESSAGES);
    }

    private buildPrompt(transcript: TranscriptMessage[], hasImage: boolean): string {
    ...
    private async runCodexExec(prompt: string, imageFilePath: string | null): Promise<string> {
