...
RSpec.describe Codex::ReplyParser do
  let(:parser) do
    described_class.new(
      default_suggested_replies: Codex::CliClient::DEFAULT_SUGGESTED_REPLIES,
      max_suggested_replies: Codex::CliClient::MAX_SUGGESTED_REPLIES
    )
  end
  ...
  describe "#parse_reply" do
    it "falls back to plain assistant text and default suggestions when json parsing fails" do
      expect(parser.parse_reply("第一段\n\n第二段")).to eq(
        text: "第一段\n\n第二段",
        suggested_replies: Codex::CliClient::DEFAULT_SUGGESTED_REPLIES
      )
    end
