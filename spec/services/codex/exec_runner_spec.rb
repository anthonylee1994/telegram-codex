...
RSpec.describe Codex::ExecRunner do
  ...
  describe "#run" do
    ...
    it "raises when codex exec fails" do
      ...
      }.to raise_error(Codex::ExecRunner::ExecutionError, "codex exec failed: boom")
    end
    ...
    it "kills codex exec and raises a timeout error when execution takes too long" do
      ...
      expect {
        runner.run(prompt: "prompt text", image_file_paths: [])
      }.to raise_error(Codex::ExecRunner::ExecutionTimeoutError, "codex exec timed out after 1 seconds")
